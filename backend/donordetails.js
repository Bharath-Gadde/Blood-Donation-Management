...
app.get("/maleDonors", (req, res) => {
  const sql =
    "SELECT COUNT(*) AS count FROM donor_details WHERE Gender = 'Male'";
  db.query(sql, (err, result) => {
    if (err) {
      console.error(err);
      return res.status(500).json({ error: "Internal Server Error" });
...
app.get("/femaleDonors", (req, res) => {
  const sql =
    "SELECT COUNT(*) AS count FROM donor_details WHERE Gender = 'Female'";
  db.query(sql, (err, result) => {
    if (err) {
      console.error(err);
      return res.status(500).json({ error: "Internal Server Error" });
...
app.post("/blood-donation", async (req, res) => {
  ...
  try {
    ...
    const sql =
      "INSERT INTO donor_details (`Fullname`,`Email`,`Password_hash`,`Gender`,`Mobilenumber`,`Blood_group`,`State`,`City`,`Home_address`,`Last_time_donated`,`Did_you_ever_donated`,`Are_you_on_medication`,`Do_you_have_disease`,`Role`) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    const values = [
      req.body.Fullname,
      req.body.Email,
...
app.post("/donation-history", (req, res) => {
  const { donor_id, donation_date, location, hospital, notes } = req.body;
  const sql = "INSERT INTO donation_history (donor_id, donation_date, location, hospital, notes) VALUES (?, ?, ?, ?, ?)";
  db.query(sql, [donor_id, donation_date, location, hospital, notes], (err, result) => {
...
app.get("/donation-history/:donor_id", (req, res) => {
  const { donor_id } = req.params;
  const sql = "SELECT * FROM donation_history WHERE donor_id = ?";
